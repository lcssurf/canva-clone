// app/api/[[...route]]/transcription.ts (ou onde você organiza suas rotas)
import { z } from "zod";
...
// Schema para validação do InstagramPost
...
const transcriptionResponseSchema = z.object({
  success: z.boolean(),
  transcriptions: z.record(z.string(), z.array(z.string())),
  failedPosts: z.array(z.string()),
...
function normalizePostMedia(post: z.infer<typeof instagramPostSchema>) {
    const images: string[] = [];
    let video: string | undefined = undefined;

    if (post?.reel) {
        return { images, video: post.videoURL || undefined };
    }
    ...
    } else if (post?.images?.length && post.images.length > 1) {
  ...
  .post(
    ...
    async (c) => {
      try {
        ...
        // Filtrar apenas posts do Instagram que precisam de transcrição
        ...
        if (instagramPosts.length === 0) {
          const response = {
            ...
            transcriptions: {},
            failedPosts: [],
            message: "Nenhum post do Instagram precisa de transcrição"
          };
          return c.json({ data: response });
        }

        console.log("📄 Posts selecionados para transcrição:", instagramPosts.map((p) => p.url).join(", "));
        ...
        for (let i = 0; i < instagramPosts.length; i += batchSize) {
          const batch = instagramPosts.slice(i, i + batchSize);
